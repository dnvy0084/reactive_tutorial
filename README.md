...
위 코드가 마이크로소프트의 엑셀이고 c 셀은 a 셀에 1을 더한 값을 가지도록 함수를 설정한거라고 보면 될 것 같네요. 당연한거지만 c가 5가되려면 ```c = a + 1```을 한번 더 호출해줘야 되는데요, 리액티브 프로그래밍이라고 다르진 않습니다. a의 값이 변경되면 c 변수를 갱신해줘야 하는건 마찬가지이지만 그 방법에 차이가 있고, 그걸 위해 스트림이라는 개념을 가져와 사용하고 있습니다. 리액티브 프로그래밍에서는 "Everything is Stream"이란 말이 있을 정도로 스트림을 이용해 외부 입출력부터 사용자 입력이나 값의 변경까지 모든것을 처리하고 있습니다. 
